          ...
          <div class="flex flex-wrap mb-6">
            <a-form layout="vertical" class="w-full">
              <a-form-item label="Camera Index">
                <a-input v-model:value="data.index" value="0" class="placeholder:text-sm shadow-sm rounded-md" />
                <span v-if="hasError('camera_index')" class="block mt-1 text-red-500 text-xs font-semibold">
                  {{ hasError("index") }}
                </span>
              </a-form-item>
              ...
              <a-form-item label="Is Active" class="w-1/2">
                <a-switch v-model:checked="data.is_active" :checked-value="1" :unchecked-value="0" />
              </a-form-item>
...
const { developers, camera, timezones } = defineProps({
  developers: Object,
  camera: Object,
  timezones: Object,
});
