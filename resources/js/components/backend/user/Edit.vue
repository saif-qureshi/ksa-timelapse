    ...
    <div>
        <div class="grid grid-cols-12 mt-8">
            <div class="col-span-12 lg:col-span-8 sm:col-span-12">
                <h2 class="text-lg font-medium truncate mr-5">Update User</h2>
            </div>
        </div>
...
const { developers, roles, user } = defineProps({
    developers: Object,
    roles: Array,
    user: Object,
});
...
const onSubmit = async () => {
    try {
        loading.value = true;
        errors.value = {};
        await axios.put(`/user/${user.id}`, data.value);
        loading.value = false;
...
const getProjectsByDevelopers = async () => {
    try {
        const { data: serverProjects } = await axios.post(`/developer/project`, {
            developers: data.value.developers,
        });
...
onMounted(async () => {
    data.value.developers = user.developers;
    data.value.projects = user.projects;
    await getProjectsByDevelopers();
});

watch(
    () => data.value.developers,
    async () => await getProjectsByDevelopers()
);
</script>
