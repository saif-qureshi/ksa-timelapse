                    ...
                    <li class="">
                        <a @if($childItem->last()->count() == 0) href="{{ $childItem->get('link') }}" @endif
                           class="side-menu @if(Route::currentRouteName() == strtolower($childItem->get('route'))) side-menu--active @endif">
                            <div class="side-menu__icon mx-0"><i data-lucide="{{ $childItem->get('icon') }}"></i></div>
                            <div class="side-menu__title">
                                {{ $childItem->first() }}
                        ...
                        @if($childItem->last()->count() > 0)
                            <ul>
                                @foreach($childItem->last() as $child)
                                    <li class="">
                                        <a href="{{ $child->get('link') }}" class="side-menu @if(Route::currentRouteName() == strtolower($child->get('route'))) side-menu--active @endif">
                                            <div class="side-menu__icon mx-0"><i data-lucide="{{ $child->get('icon') }}"></i></div>
